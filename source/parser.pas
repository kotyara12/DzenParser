...
type
  ...
  public
    ...
    function GetTextRequsive: string;
...
begin
 if self.TypeTag='</%s>' then
     result:= format(AnsiReplaceStr(self.TypeTag,'/',''),[Trim(self.Tag  + ' ' +  self.AttributesTxt)] )
 else
     result:= format(self.TypeTag,[Trim(self.Tag  + ' ' +  self.AttributesTxt)] );
end;
...
function TDomTreeNode.GetTextRequsive: string;
var
  i: integer;
begin
  result:=self.Text;
  for i := 0 to self.Child.Count - 1 do
  begin
    result := self.Child[i].GetTextRequsive;
    if result <> '' then
      exit;
  end;
end;
